"""
Command-Line Interface for Rowhammer DRAM Bit-Flip Vulnerability & TRR Mitigation Agent.
"""
import argparse
import csv
import json
...
from .agents import RowhammerCoordinator

coordinator = RowhammerCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="rowhammer-dram-mitigation-agent", description="Rowhammer DRAM Bit-Flip Vulnerability & TRR Mitigation Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  ROWHAMMER DRAM BIT-FLIP VULNERABILITY & TRR MITIGATION AGENT")
        print(f"  Domain: Hardware Security | Standard: JEDEC DRAM Security Guidelines")
    ...
    if args.command == "batch":
        ...
        for idx, r in enumerate(rows):
            try:
                ...
                print(f"Warning: Skipping row {idx+1} due to invalid data: {e}", file=sys.stderr)
                errors += 1
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting Rowhammer DRAM Bit-Flip Vulnerability & TRR Mitigation Agent on http://{args.host}:{args.port}")
